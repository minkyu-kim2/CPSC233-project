...
public class ItemPageController extends PageController {
	...
	/**
	 * on clicking the delete button, the item is removed from the shopping list and
	 * the data.txt, and the user is taken back to the ShoppingListPage.
	 * 
	 * @param event
	...
	/**
	 * This function is fired when the edit button is clicked. On clicking the
	 * function, the user is taken to the category selection page.
	 * 
	 * @param event
	 * @throws FileNotFoundException
	 * @throws IOException
	 */
	@FXML
	public void edit(ActionEvent event) throws FileNotFoundException, IOException {
		Scene currentScene = applicationStage.getScene();
		String currentTitle = applicationStage.getTitle();
		CategorySelectionPageController controller = (CategorySelectionPageController) loadPage(item,
				CategorySelectionPageController.getPathToFxml(), applicationStage, shoppingList, "create a new item");
